// SOURCE FROM: https://github.com/grihlo/stacked_card_carousel/
// BY: Grigori Hlopkov
import 'dart:math';
...
class _StackedCardCarouselState extends State<StackedCardCarousel> {
  ...
  Widget _stackedCards(BuildContext context) {
    ...
    final List<Widget> _positionedCards = widget._items.asMap().entries.map(
      (MapEntry<int, Widget> item) {
        ...
        if (_pageValue < item.key) {
          position += (_pageValue - item.key) * widget._spaceBetweenItems * textScaleFactor;
        }
        switch (widget._type) {
    ...
    return Stack(alignment: Alignment.center, fit: StackFit.passthrough, children: _positionedCards);
  }
}
