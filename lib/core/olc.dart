...
/// First place value of the pairs
...
class OLC {
  ...
  static num clipLatitude(num latitude) => latitude.clamp(-90.0, 90.0);
  ...
  /// columns than rows.
  static num computeLatitudePrecision(int codeLength) {
    if (codeLength <= 10) {
  ...
  static num normalizeLongitude(num longitude) {
    while (longitude < -180) {
      longitude += 360;
    }
    while (longitude >= 180) {
  ...
  static bool isShort(String code) {
    // Check it's valid.
    if (!isValid(code)) {
      return false;
    }
  ...
  static bool isFull(String code) {
    if (!isValid(code)) {
      return false;
    }
  ...
  static String encode(num latitude, num longitude,
      {int codeLength = pairCodeLength}) {
  ...
  ///
  /// Returns a [CodedPosition] object that includes the coordinates of the bounding
  /// box - the lower left, center and upper right.
  static CodedPosition decode(String code) {
    if (!isFull(code)) {
      throw ArgumentError(
    ...
    return CodedPosition(lat, lng, lat + latPrecision, lng + lngPrecision,
  ...
  /// close enough, or the .
  static String shorten(String code, num latitude, num longitude) {
    if (!isFull(code)) {
...
class CodedPosition {
  ...
  CodedPosition(num south, num west, num north, num east, this.codeLength)
      : south = south,
        west = west,
        north = north,
        east = east,
